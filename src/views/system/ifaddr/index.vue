<template>
  <n-card :bordered="false" class="proCard">
    ...
    <n-button type="info" ghost @click="addAdminip"
      >添加
      <template #icon>
        <n-icon>
          <PlusOutlined />
    ...
    <n-modal
      v-model:show="adminipControl.editShow"
      preset="dialog"
      title="Dialog"
      :mask-closable="false"
      style="width: 600px"
    >
      ...
      <div>
        <editAdminip
          ref="adminipEditRef"
          :adminipInfo="adminipInfo"
          :adminipId="adminipControl.adminipId"
          :isAdd="adminipControl.isAdd"
        />
      </div>
      <template #action>
        <n-space>
...
<script lang="ts" setup>
  ...
  const loadDataTable = async (res) => {
    let userList = await getAdminipList({ ...params, ...res });
